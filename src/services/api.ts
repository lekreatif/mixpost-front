...
import { IUser, SocialAccount, Page, ApiResponse } from '@/types'
...
export const login = async (
  ...
  password: string,
  rememberMe: boolean
): Promise<AxiosResponse<{ data: { isAuthenticated: boolean } }>> => {
...
export const getAuthState = (): Promise<
  AxiosResponse<ApiResponse<{ isAuthenticated: boolean }>>
> => api.get('/auth/auth-state')
export const getMyPages = (): Promise<AxiosResponse<ApiResponse<Page[]>>> =>
  api.get('/pages/my-pages')
export const getUser: () => Promise<
  AxiosResponse<ApiResponse<IUser>>
> = async () => api.get('/user/me')

export const getFacebookAuthUrl = async (): Promise<
...
export const getSocialAccounts = async (): Promise<
  AxiosResponse<ApiResponse<SocialAccount[]>>
...
export const updateSocialAccount = async (
  ...
  } = await api.put<AxiosResponse<ApiResponse<SocialAccount>>>(
